...
if(isServer) then {
	...
	if (isClass (configfile >> "CfgPatches" >> "ace_medical")) then {
		AIS_REVIVE_GUARANTY = 0; publicVariable "AIS_REVIVE_GUARANTY"; hint "If AIS still active then dont use ace medical";
	} else {
		//GET_PARAM(bis_reviveParam_mode, "ReviveMode", 1);
		GET_PARAM_BOOL(AIS_REVIVE_GUARANTY, "AISREVIVEGUARANTY",1);
	};
	...
	GET_PARAM(AIS_REVIVE_INIT_UNITS, "AISREVIVEINITUNITS",1);
	GET_PARAM(AIS_MEDICAL_EDUCATION, "AISMEDICALEDUCATION",1);
	GET_PARAM_BOOL(AIS_REVIVE_GUARANTY, "AISREVIVEGUARANTY",1);
	GET_PARAM(AIS_BLEEDOUT_TIME, "AISBLEEDOUTTIME",300);
	GET_PARAM(AIS_REVIVETIME, "AIS_REVIVETIME",30);
	GET_PARAM(AIS_STABILIZETIME, "AIS_STABILIZETIME",20);
	GET_PARAM_BOOL(AIS_REVIVE_HEAL, "AIS_REVIVE_HEAL",1);
	GET_PARAM_BOOL(AIS_TOGGLE_RADIO, "AIS_TOGGLE_RADIO",1);
	GET_PARAM_BOOL(AIS_NO_CHAT, "AIS_NO_CHAT",1);
	GET_PARAM(AIS_AI_HELP_RADIUS, "AIS_AI_HELP_RADIUS",100);
	//GET_PARAM(GRLIB_halo_altitude, "GRLIB_halo_altitude",1);
...
if (!isDedicated && hasInterface) then {
    ...
    if (bis_reviveParam_mode == 1) then {
        ...
        _text = _text + format ["<font color='#ff8000'>%1</font><br />%2<br /><br />", _param, _value];
		//-------------------------------------------------------------------------AIS added by luke ----------------------------------------------------------------------------------------------------------------------------------
		_param = localize "STR_AIS_REVIVE_GUARANTY";
        _value = str AIS_REVIVE_GUARANTY;
        _text = _text + format ["<font color='#ff8000'>%1</font><br />%2<br /><br />", _param, _value];
		
		_param = localize "STR_AIS_REVIVE_INIT_UNITS;";
        _value = str AIS_REVIVE_INIT_UNITS;
        _text = _text + format ["<font color='#ff8000'>%1</font><br />%2<br /><br />", _param, _value];
		
		_param = localize "STR_AIS_MEDICAL_EDUCATION;";
        _value = str AIS_MEDICAL_EDUCATION;
        _text = _text + format ["<font color='#ff8000'>%1</font><br />%2<br /><br />", _param, _value];
		
		_param = localize "STR_AIS_BLEEDOUT_TIME;";
        _value = str AIS_BLEEDOUT_TIME;
        _text = _text + format ["<font color='#ff8000'>%1</font><br />%2<br /><br />", _param, _value];
		
		_param = localize "STR_AIS_REVIVETIME;";
        _value = str AIS_REVIVETIME;
        _text = _text + format ["<font color='#ff8000'>%1</font><br />%2<br /><br />", _param, _value];
			
		_param = localize "STR_AIS_STABILIZETIME;";
        _value = str AIS_STABILIZETIME;
        _text = _text + format ["<font color='#ff8000'>%1</font><br />%2<br /><br />", _param, _value];
